...
ompl::base::Cost tesseract_motion_planners::ClearanceObjective::motionCost(const ompl::base::State* s1, const ompl::base::State* s2) const
{
  ...
  std::printf("Start:  %+4f %+4f %+4f %+4f %+4f %+4f  Finish: %+4f %+4f %+4f %+4f %+4f %+4f ",
              start->values[0], start->values[1], start->values[2], start->values[3], start->values[4], start->values[5],
              finish->values[0], finish->values[1], finish->values[2], finish->values[3], finish->values[4], finish->values[5]);
  ...
  if (result_it == contact_map.end())
  {
    std::cout << "Clearance > 0.2 Cost = 0" << std::endl;
    return ompl::base::Cost(0.0);  // return zero cost if so far away from collision that zero contacts are reported
  }

  double clearance_clamped = std::max(static_cast<double>(result_it->second[0].distance), 0.000001);  // enforce that collision clearance should be nonzero and positive to calculate a valid cost

  double cost = 1 / clearance_clamped;

  std::cout << "Clearance = " << clearance_clamped << " Cost = " << cost << std::endl;
