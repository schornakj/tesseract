...
#include <tesseract_planning/trajopt/trajopt_spline_planner.h>
...
bool TrajOptSplinePlanner::solve(PlannerResponse& response, const TrajOptSplinePlannerConfig& config)
{
  // Check that parameters are valid
  if (config.env_ == nullptr)
    throw std::invalid_argument("In trajopt_spline_planner: env_ is a required parameter and has not been set");
  if (config.kin_ == nullptr)
    throw std::invalid_argument("In trajopt_spline_planner: kin_ is a required parameter and has not been set");
  ...
  for (std::size_t ind = 0; ind < config.target_waypoints_.size(); ind++)
  {
    ...
    switch (waypoint_type)
    {
      ...
      case tesseract::tesseract_planning::WaypointType::CARTESIAN_WAYPOINT:
      {
        ...
        pose->pos_coeffs = config.pos_coeffs_;
        pose->rot_coeffs = config.rot_coeffs_;
...
bool TrajOptSplinePlanner::terminate() { return false; }
void TrajOptSplinePlanner::clear() { request_ = PlannerRequest(); }
}  // namespace tesseract_planning
}  // namespace tesseract
